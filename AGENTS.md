...
- `ensureSchema` is purely additive (`CREATE TABLE/INDEX IF NOT EXISTS`, never `DROP`). If local D1 predates a schema change, run `pnpm run db:reset:local` (wipes only `apps/web/.wrangler/state/v3/d1`, then reapplies `0001_init.sql`) — never `--remote`. An existing **remote** D1 from the pre-onboarding `0001_init.sql` will not re-run that rewritten migration; it needs a one-time wipe/recreate (or a future `0002`). `db:reset:local` never touches remote.
