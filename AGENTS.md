...
- v1 is one Sleeper league. `V1_LEAGUE_ID` is a wrangler var sourced from gitignored / 1Password config (`Cutman (dev)`), never a live id in tracked files. Do not grow past one league.
- Access is an allowlist (Clerk email → Sleeper `user_id`) plus a live membership check on `/league/.../users`. No claim flow. No FF-XXXX. No magic-link.
